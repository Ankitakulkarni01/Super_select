...
const HeaderNavigationMenu = (props) => {
    console.log(props)
  const [activeHeaderValue, setActiveHeaderVal] = useState(props.activeValue ? props.activeValue : props?.menu[0]?.value)
  ...
  return (
    ...
    keyExtractor={(item, index) => index.toString()}
        ...
        renderItem={({ item, index }: { item: NavigationItem ; index: number }) => {
            console.log(activeHeaderValue === item.value);
          ...
          <Pressable
            key={item.key}
            style={[styles.navItem, activeHeaderValue === item.value && styles.navItemActive]}
